...
import com.bitchat.android.services.ConversationSummaryStore
import com.bitchat.android.summary.ConversationSummaryEngine
import com.bitchat.android.summary.ConversationSummaryKeys
import com.bitchat.android.summary.ConversationSummaryKind
import com.bitchat.android.summary.ConversationSummarySheetData
import com.bitchat.android.summary.SummaryInputMessage
...
class ChatViewModel(
    ...
    private val conversationSummaryEngine = ConversationSummaryEngine()
    private val conversationSummaryStore = ConversationSummaryStore.getInstance(application.applicationContext)
    private val lastOpenedConversationAnchors = mutableMapOf<String, Long?>()
    ...
    private val groupManager = GroupManager(state, messageManager, channelManager, dataManager)
    ...
    val groupInfoSheetGroupId: StateFlow<String?> = state.groupInfoSheetGroupId
    val conversationSummarySheet: StateFlow<ConversationSummarySheetData?> = state.conversationSummarySheet
    ...
    val groupInfoMap: StateFlow<Map<String, GroupInfo>> = state.groupInfoMap

    init {
        // Note: Mesh service delegate is now set by MainActivity
        com.bitchat.android.services.AppStateStore.initialize(getApplication())
        com.bitchat.android.services.AppStateStore.setLocalIdentity(meshService.myPeerID, null)
        notificationManager.setCurrentNickname(null)
        loadAndInitialize()
    ...
    private fun loadAndInitialize() {
        ...
        try { com.bitchat.android.services.AppStateStore.setLocalIdentity(meshService.myPeerID, nickname) } catch (_: Exception) { }
        notificationManager.setCurrentNickname(nickname)
        ...
        // Restore persisted group chats
        groupManager.loadPersistedGroups(meshService.myPeerID)

        markCurrentVisibleConversationOpened()
    ...
    fun setNickname(newNickname: String) {
        ...
        try { com.bitchat.android.services.AppStateStore.setLocalIdentity(meshService.myPeerID, newNickname) } catch (_: Exception) { }
        notificationManager.setCurrentNickname(newNickname)
    ...
    fun createGroup(name: String, selectedPeerIDs: List<String>) {
        ...
        val groupInfo = groupManager.createGroup(
            name = name,
        ...
        switchToChannel(groupInfo.id)
    }

    fun leaveGroup(groupID: String) {
        val existing = groupManager.getGroupInfo(groupID) ?: return
        val updatedGroup = groupManager.leaveGroup(groupID, meshService.myPeerID)
        hideGroupInfoSheet()
        routeGroupControlUpdate(
            groupID = groupID,
            payload = updatedGroup?.let {
                groupManager.buildControlPayload(
                    groupInfo = it,
                    action = GroupManager.ACTION_MEMBER_LEFT,
                    actorPeerID = meshService.myPeerID,
                    targetPeerID = meshService.myPeerID,
                    targetNickname = existing.memberNicknames[meshService.myPeerID]
                        ?: state.getNicknameValue()
                )
            },
            recipientPeerIDs = existing.memberPeerIDs.filter { it != meshService.myPeerID },
            messageID = java.util.UUID.randomUUID().toString()
        )
    }

    fun updateGroupDetails(groupID: String, name: String, description: String): Boolean {
        val updated = groupManager.updateGroupDetails(
            groupID = groupID,
            name = name,
        ...
        routeGroupControlUpdate(
            groupID = groupID,
            payload = groupManager.buildControlPayload(
                groupInfo = updated,
                action = GroupManager.ACTION_METADATA_UPDATED,
                actorPeerID = meshService.myPeerID
            ),
            recipientPeerIDs = updated.memberPeerIDs.filter { it != meshService.myPeerID },
            messageID = java.util.UUID.randomUUID().toString()
        )
        return true
    }

    fun addGroupMembers(groupID: String, peerIDs: List<String>): Boolean {
        val addedPeerIDs = peerIDs
            .distinct()
            .filterNot { groupManager.getGroupInfo(groupID)?.memberPeerIDs?.contains(it) == true }
        if (addedPeerIDs.isEmpty()) return false

        val updated = groupManager.addMembers(
            groupID = groupID,
            peerIDs = addedPeerIDs,
            actorPeerID = meshService.myPeerID,
            peerNicknames = meshService.getPeerNicknames() + mapOf(
                meshService.myPeerID to (state.getNicknameValue() ?: meshService.myPeerID)
            )
        ) ?: return false
        ...
        routeGroupControlUpdate(
            ...
            payload = groupManager.buildControlPayload(
                groupInfo = updated,
                action = GroupManager.ACTION_MEMBERSHIP_UPDATED,
                actorPeerID = meshService.myPeerID,
                targetPeerID = addedPeerIDs.singleOrNull(),
                targetNickname = addedPeerIDs.singleOrNull()?.let { updated.memberNicknames[it] }
            ),
            recipientPeerIDs = updated.memberPeerIDs
                .filter { it != meshService.myPeerID && !addedPeerIDs.contains(it) },
            messageID = java.util.UUID.randomUUID().toString()
        )
        return true
    }
    ...
    fun removeGroupMember(groupID: String, targetPeerID: String): Boolean {
        val existing = groupManager.getGroupInfo(groupID) ?: return false
        val targetNickname = existing.memberNicknames[targetPeerID]
        val updated = groupManager.removeMember(
            groupID = groupID,
        ...
        routeGroupControlUpdate(
            ...
            payload = groupManager.buildControlPayload(
                groupInfo = updated,
                action = GroupManager.ACTION_MEMBER_REMOVED,
                actorPeerID = meshService.myPeerID,
                targetPeerID = targetPeerID,
                targetNickname = targetNickname
            ),
            recipientPeerIDs = (updated.memberPeerIDs + targetPeerID)
                .filter { it != meshService.myPeerID }
                .distinct(),
            messageID = java.util.UUID.randomUUID().toString()
        )
        return true
    ...
    fun isGroupAdmin(groupID: String): Boolean {
        return groupManager.isAdmin(groupID, meshService.myPeerID)
    }
    ...
    fun startPrivateChat(peerID: String) {
        ...
        if (success) {
            markConversationOpened(ConversationSummaryKeys.direct(peerID))
    ...
    fun sendMessage(content: String) {
        ...
        if (currentChannelValue != null && GroupManager.isGroupChannel(currentChannelValue)) {
            sendGroupMessage(currentChannelValue, content)
            return
        }
    ...
    private fun sendGroupMessage(groupID: String, content: String) {
        val groupInfo = groupManager.getGroupInfo(groupID) ?: return
        ...
        val senderName = state.getNicknameValue() ?: meshService.myPeerID
        ...
        channelManager.addChannelMessage(groupID, localMessage, meshService.myPeerID)
    ...
    private fun routeGroupControlUpdate(
        groupID: String,
        payload: GroupManager.GroupControlPayload?,
        recipientPeerIDs: List<String>,
        messageID: String
    ) {
        val groupInfo = payload?.let { groupManager.getGroupInfo(groupID) ?: GroupInfo(
            id = it.groupID,
            name = it.name,
            description = it.description,
            memberPeerIDs = it.memberPeerIDs,
            memberNicknames = it.memberNicknames,
            adminPeerIDs = it.adminPeerIDs,
            createdByPeerID = it.createdByPeerID,
    ...
    fun showGroupInfoSheet(groupID: String) {
        state.setGroupInfoSheetGroupId(groupID)
    }

    fun hideGroupInfoSheet() {
        state.setGroupInfoSheetGroupId(null)
    }
    ...
    fun panicClearAllData() {
        ...
        conversationSummaryStore.clear()
        lastOpenedConversationAnchors.clear()
        hideConversationSummarySheet()
    ...
    private fun recreateMeshServiceAfterPanic() {
        ...
        try { com.bitchat.android.services.AppStateStore.setLocalIdentity(meshService.myPeerID, state.getNicknameValue()) } catch (_: Exception) { }
    ...
    fun showCurrentConversationSummary() {
        buildCurrentConversationSelection()?.let { selection ->
            state.setConversationSummarySheet(buildSummarySheetData(selection))
        }
    }

    fun showPrivateConversationSummary(peerID: String) {
        val selection = buildPrivateConversationSelection(peerID) ?: return
        state.setConversationSummarySheet(buildSummarySheetData(selection))
    }

    fun hideConversationSummarySheet() {
        state.setConversationSummarySheet(null)
    }
    ...
    fun handleBackPressed(): Boolean {
        return when {
            ...
            state.getConversationSummarySheetValue() != null -> {
                hideConversationSummarySheet()
                true
            }
            // Exit private chat
            ...
            state.getGroupInfoSheetGroupIdValue() != null -> {
                hideGroupInfoSheet()
                true
            }
            // Exit channel view
    ...
    private fun markCurrentVisibleConversationOpened() {
        buildCurrentConversationSelection()?.let { markConversationOpened(it.key) }
    }

    private fun markConversationOpened(conversationKey: String) {
        val previous = conversationSummaryStore.markOpened(conversationKey)
        lastOpenedConversationAnchors[conversationKey] = previous
    }

    private fun buildSummarySheetData(selection: ConversationSelection): ConversationSummarySheetData {
        val summary = conversationSummaryEngine.summarizeConversation(
            messages = selection.messages.map { it.toSummaryInputMessage() },
            currentUserNickname = state.getNicknameValue()
        )
        val catchUpMessages = lastOpenedConversationAnchors[selection.key]
            ?.let { anchor -> selection.messages.filter { it.timestamp.time > anchor } }
            .orEmpty()
        val catchUpSummary = if (catchUpMessages.isNotEmpty()) {
            conversationSummaryEngine.summarizeConversation(
                messages = catchUpMessages.map { it.toSummaryInputMessage() },
                currentUserNickname = state.getNicknameValue()
            )
        } else {
            null
        ...
        return ConversationSummarySheetData(
    ...
    private fun buildCurrentConversationSelection(): ConversationSelection? {
        val selectedPeer = state.getSelectedPrivateChatPeerValue()
        if (selectedPeer != null) {
            return buildPrivateConversationSelection(selectedPeer)
        }

        val currentChannel = state.getCurrentChannelValue()
        if (currentChannel != null) {
            val isGroup = GroupManager.isGroupChannel(currentChannel)
            return ConversationSelection(
                key = ConversationSummaryKeys.channel(currentChannel),
                title = if (isGroup) {
                    state.getGroupInfoMapValue()[currentChannel]?.name ?: currentChannel
                } else {
                    currentChannel
                },
                kind = if (isGroup) ConversationSummaryKind.GROUP else ConversationSummaryKind.CHANNEL,
                messages = state.getChannelMessagesValue()[currentChannel].orEmpty()
            )
        }

        val selectedLocation = state.selectedLocationChannel.value
        return if (selectedLocation is com.bitchat.android.geohash.ChannelID.Location) {
            val channelID = "geo:${selectedLocation.channel.geohash}"
            ConversationSelection(
                key = ConversationSummaryKeys.channel(channelID),
                title = "#${selectedLocation.channel.geohash}",
                kind = ConversationSummaryKind.GEOHASH,
                messages = state.getChannelMessagesValue()[channelID].orEmpty()
            )
        } else {
            ConversationSelection(
                key = ConversationSummaryKeys.global(),
                title = "Mesh Chat",
                kind = ConversationSummaryKind.GLOBAL,
                messages = state.getMessagesValue()
            )
        }
    }

    private fun buildPrivateConversationSelection(peerID: String): ConversationSelection? {
        val messages = state.getPrivateChatsValue()[peerID].orEmpty()
        val title = meshService.getPeerNicknames()[peerID]
            ?: peerID.take(12)
        return ConversationSelection(
            key = ConversationSummaryKeys.direct(peerID),
            title = title,
            kind = ConversationSummaryKind.DIRECT,
            messages = messages
        )
    }

    private fun BitchatMessage.toSummaryInputMessage(): SummaryInputMessage {
