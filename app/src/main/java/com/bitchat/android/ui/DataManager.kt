...
class DataManager(private val context: Context) {
    ...
    private val _groupInfoMap = mutableMapOf<String, GroupInfo>()
    ...
    val groupInfoMap: Map<String, GroupInfo> get() = _groupInfoMap
    ...
    fun loadGroups(): Map<String, GroupInfo> {
        val groupsJson = prefs.getString("group_info_map", "{}") ?: "{}"
        return try {
            val listType = object : TypeToken<List<Map<String, Any?>>>() {}.type
            val rawGroups: List<Map<String, Any?>> = gson.fromJson(groupsJson, listType) ?: emptyList()
            val parsed = rawGroups
                .mapNotNull(::parseStoredGroup)
                .associateBy { it.id }
            _groupInfoMap.clear()
            _groupInfoMap.putAll(parsed)
            parsed
        } catch (e: Exception) {
            Log.w(TAG, "Failed to load groups: ${e.message}")
            emptyMap()
        }
    }

    fun saveGroups(groups: Map<String, GroupInfo>) {
        _groupInfoMap.clear()
        _groupInfoMap.putAll(groups)
        prefs.edit()
            .putString("group_info_map", gson.toJson(groups.values.toList()))
            .apply()
    }

    fun saveGroup(groupInfo: GroupInfo) {
        val updated = _groupInfoMap.toMutableMap()
        updated[groupInfo.id] = groupInfo
        saveGroups(updated)
    }
    ...
    private fun parseStoredGroup(raw: Map<String, Any?>): GroupInfo? {
        val id = raw["id"] as? String ?: return null
        val name = raw["name"] as? String ?: return null
        val description = raw["description"] as? String ?: ""
        val memberPeerIDs = (raw["memberPeerIDs"] as? List<*>)
            ?.mapNotNull { it as? String }
            ?.distinct()
            .orEmpty()
        val memberNicknames = (raw["memberNicknames"] as? Map<*, *>)
            ?.mapNotNull { (key, value) ->
                val peerID = key as? String ?: return@mapNotNull null
                val nickname = value as? String ?: return@mapNotNull null
                peerID to nickname
            }
            ?.toMap()
            .orEmpty()
        val createdByPeerID = raw["createdByPeerID"] as? String ?: memberPeerIDs.firstOrNull() ?: return null
        val createdAtMs = (raw["createdAtMs"] as? Number)?.toLong() ?: System.currentTimeMillis()
        val updatedAtMs = (raw["updatedAtMs"] as? Number)?.toLong() ?: createdAtMs
        val adminPeerIDs = (raw["adminPeerIDs"] as? List<*>)
            ?.mapNotNull { it as? String }
            ?.filter { memberPeerIDs.contains(it) }
            ?.distinct()
            .orEmpty()
            .ifEmpty { listOf(createdByPeerID).filter { memberPeerIDs.contains(it) } }
