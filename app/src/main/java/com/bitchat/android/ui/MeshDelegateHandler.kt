...
class MeshDelegateHandler(
    ...
    override fun didReceiveMessage(message: BitchatMessage) {
        coroutineScope.launch {
            ...
            if (message.isPrivate) {
                ...
                if (channel != null && GroupManager.isGroupChannel(channel) && !groupManager.isKnownGroup(channel)) {
                    return@launch
                }
    ...
    private fun handleGroupInvite(message: BitchatMessage): Boolean {
        if (!message.content.startsWith(GroupManager.INVITE_PREFIX)) return false
        val payload = runCatching {
            gson.fromJson(
                message.content.removePrefix(GroupManager.INVITE_PREFIX),
                GroupManager.GroupInvitePayload::class.java
            )
        }.getOrNull() ?: return false

        groupManager.joinGroupFromInvite(
            payload = payload,
            myPeerID = getMyPeerID(),
            inviterName = message.sender
        )
        return true
    }

    private fun handleGroupControl(message: BitchatMessage): Boolean {
        if (!message.content.startsWith(GroupManager.CONTROL_PREFIX)) return false
        val payload = runCatching {
            gson.fromJson(
                message.content.removePrefix(GroupManager.CONTROL_PREFIX),
                GroupManager.GroupControlPayload::class.java
            )
        }.getOrNull() ?: return false

        groupManager.applyControlMessage(
            payload = payload,
            myPeerID = getMyPeerID(),
