...
class BluetoothMeshService(private val context: Context) {
    ...
    init {
        ...
        try {
            com.bitchat.android.services.AppStateStore.initialize(context.applicationContext)
            val nickname = try { com.bitchat.android.services.NicknameProvider.getNickname(context, myPeerID) } catch (_: Exception) { myPeerID }
            com.bitchat.android.services.AppStateStore.setLocalIdentity(myPeerID, nickname)
        } catch (_: Exception) { }
