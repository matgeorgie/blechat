...
import com.bitchat.android.ui.GroupManager
...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
            ...
            when (noisePayload.type) {
                com.bitchat.android.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    ...
                    if (privateMessage != null) {
                        ...
                        decodeGroupInvite(pmContent)?.let {
                        ...
                        decodeGroupMessage(pmContent)?.let { groupMessage ->
                            val message = BitchatMessage(
                                ...
                                isPrivate = false,
                                senderPeerID = peerID,
                                channel = groupMessage.groupID
                            )
                            delegate?.onMessageReceived(message)
                            sendDeliveryAck(groupMessage.messageID, peerID)
                            return
                        }

                        decodeGroupControl(pmContent)?.let {
                            val message = BitchatMessage(
                                id = privateMessage.messageID,
                                sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                                content = pmContent,
    ...
    private fun decodeGroupInvite(content: String): GroupManager.GroupInvitePayload? {
        if (!content.startsWith(GroupManager.INVITE_PREFIX)) return null
        return runCatching {
            gson.fromJson(
                content.removePrefix(GroupManager.INVITE_PREFIX),
                GroupManager.GroupInvitePayload::class.java
            )
        }.getOrNull()
    }

    private fun decodeGroupMessage(content: String): GroupManager.GroupMessagePayload? {
        if (!content.startsWith(GroupManager.MESSAGE_PREFIX)) return null
        return runCatching {
            gson.fromJson(
                content.removePrefix(GroupManager.MESSAGE_PREFIX),
                GroupManager.GroupMessagePayload::class.java
            )
        }.getOrNull()
    }

    private fun decodeGroupControl(content: String): GroupManager.GroupControlPayload? {
        if (!content.startsWith(GroupManager.CONTROL_PREFIX)) return null
        return runCatching {
            gson.fromJson(
                content.removePrefix(GroupManager.CONTROL_PREFIX),
                GroupManager.GroupControlPayload::class.java
            )
        }.getOrNull()
    }
}
